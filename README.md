...
- Under Output Layer, click the ... button and save the file as .tif.
- A new raster layer will be generated. Right-click the newly generated difference layer in the Layers Panel and select Properties.
- Select the Symbology tab on the left menu.
- Set Render type to Singleband pseudocolor and choose a diverging color ramp (PiYG)
- Click Apply, then OK.
...
- Input Layer: sampled_points and Raster Layer: Select output_NDVI.tif (Name output column NDVI).
- Click Run.
- Repeat the tool using the new points output layer, sampling output_NBR.tif (Name output column NBR).
